...
class NewSetting_Window(QMainWindow):
    ...
    def __init__(self,remoteXLApp,parent=None,wait_loop=None,show_run_button=True):
        ...
        self.add_known_settings_to_comboBoxes()
        self.show()
    ...
    def _get_data(self):
        ...
        if self.remote_settings:
            setting_data.update({'user':str(self.ui.username_comboBox.currentText().strip())})
            setting_data.update({'host':str(self.ui.host_comboBox.currentText().strip())})
            setting_data.update({'shelxlpath':str(self.ui.shelxlpath_comboBox.currentText().strip())})
            setting_data.update({'queingsystem':{'name':str(self.ui.queuingsystem_comboBox.currentData()['Name'])}})
            setting_data['queingsystem'].update({'displayname':str(self.ui.queuingsystem_comboBox.currentData()['Displayname'])})
            for setting in self.queue_data:
            
                if setting['Type'] == 'LineEdit':
                    setting_data['queingsystem'].update({setting['Name'] :str(setting['QObject'].text().strip())})                    
                elif setting['Type'] == 'SpinBox':
                    setting_data['queingsystem'].update({setting['Name'] :setting['QObject'].value()})    
                elif setting['Type'] == 'ComboBox':
                    setting_data['queingsystem'].update({setting['Name'] :str(setting['QObject'].currentText().strip())})  
                elif setting['Type'] == 'WalltimeWidget':
                    setting_data['queingsystem'].update({setting['Name'] :setting['QObject'].get_walltime()}) 
        else:
